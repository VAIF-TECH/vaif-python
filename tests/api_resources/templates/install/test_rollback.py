...
class TestRollback:
    ...
    def test_method_rollback(self, client: Vaif) -> None:
        rollback = client.templates.install.rollback.rollback(
            "installId",
        )
    ...
    def test_raw_response_rollback(self, client: Vaif) -> None:
        response = client.templates.install.rollback.with_raw_response.rollback(
            "installId",
        )
    ...
    def test_streaming_response_rollback(self, client: Vaif) -> None:
        with client.templates.install.rollback.with_streaming_response.rollback(
            "installId",
        ) as response:
    ...
    def test_path_params_rollback(self, client: Vaif) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `install_id` but received ''"):
            client.templates.install.rollback.with_raw_response.rollback(
                "",
            )
...
class TestAsyncRollback:
    ...
    async def test_method_rollback(self, async_client: AsyncVaif) -> None:
        rollback = await async_client.templates.install.rollback.rollback(
            "installId",
        )
        assert rollback is None
    ...
    async def test_raw_response_rollback(self, async_client: AsyncVaif) -> None:
        response = await async_client.templates.install.rollback.with_raw_response.rollback(
            "installId",
        )
    ...
    async def test_streaming_response_rollback(self, async_client: AsyncVaif) -> None:
        async with async_client.templates.install.rollback.with_streaming_response.rollback(
            "installId",
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"
    ...
    async def test_path_params_rollback(self, async_client: AsyncVaif) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `install_id` but received ''"):
            await async_client.templates.install.rollback.with_raw_response.rollback(
                "",
            )
