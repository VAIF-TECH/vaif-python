...
class TestProviders:
    ...
    def test_streaming_response_list(self, client: Vaif) -> None:
        with client.auth.oauth.providers.with_streaming_response.list() as response:
...
class TestAsyncProviders:
    ...
    async def test_method_list(self, async_client: AsyncVaif) -> None:
        provider = await async_client.auth.oauth.providers.list()
        assert_matches_type(ProviderListResponse, provider, path=["response"])

    @parametrize
    async def test_raw_response_list(self, async_client: AsyncVaif) -> None:
        response = await async_client.auth.oauth.providers.with_raw_response.list()
    ...
    async def test_streaming_response_list(self, async_client: AsyncVaif) -> None:
        async with async_client.auth.oauth.providers.with_streaming_response.list() as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"
