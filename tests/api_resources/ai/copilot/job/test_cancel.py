...
class TestCancel:
    ...
    def test_method_cancel(self, client: Vaif) -> None:
        cancel = client.ai.copilot.job.cancel.cancel(
            "jobId",
        )
    ...
    def test_raw_response_cancel(self, client: Vaif) -> None:
        response = client.ai.copilot.job.cancel.with_raw_response.cancel(
            "jobId",
        )
    ...
    def test_streaming_response_cancel(self, client: Vaif) -> None:
        with client.ai.copilot.job.cancel.with_streaming_response.cancel(
            "jobId",
        ) as response:
    ...
    def test_path_params_cancel(self, client: Vaif) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `job_id` but received ''"):
            client.ai.copilot.job.cancel.with_raw_response.cancel(
                "",
            )
...
class TestAsyncCancel:
    ...
    async def test_method_cancel(self, async_client: AsyncVaif) -> None:
        cancel = await async_client.ai.copilot.job.cancel.cancel(
            "jobId",
        )
        assert cancel is None
    ...
    async def test_raw_response_cancel(self, async_client: AsyncVaif) -> None:
        response = await async_client.ai.copilot.job.cancel.with_raw_response.cancel(
            "jobId",
        )
    ...
    async def test_streaming_response_cancel(self, async_client: AsyncVaif) -> None:
        async with async_client.ai.copilot.job.cancel.with_streaming_response.cancel(
            "jobId",
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"
    ...
    async def test_path_params_cancel(self, async_client: AsyncVaif) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `job_id` but received ''"):
            await async_client.ai.copilot.job.cancel.with_raw_response.cancel(
                "",
            )
