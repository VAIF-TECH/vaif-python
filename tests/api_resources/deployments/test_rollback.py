...
class TestRollback:
    ...
    def test_method_rollback(self, client: Vaif) -> None:
        rollback = client.deployments.rollback.rollback(
            "deploymentId",
        )
    ...
    def test_raw_response_rollback(self, client: Vaif) -> None:
        response = client.deployments.rollback.with_raw_response.rollback(
            "deploymentId",
        )
    ...
    def test_streaming_response_rollback(self, client: Vaif) -> None:
        with client.deployments.rollback.with_streaming_response.rollback(
            "deploymentId",
        ) as response:
    ...
    def test_path_params_rollback(self, client: Vaif) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `deployment_id` but received ''"):
            client.deployments.rollback.with_raw_response.rollback(
                "",
            )
...
class TestAsyncRollback:
    ...
    async def test_method_rollback(self, async_client: AsyncVaif) -> None:
        rollback = await async_client.deployments.rollback.rollback(
            "deploymentId",
        )
        assert rollback is None
    ...
    async def test_raw_response_rollback(self, async_client: AsyncVaif) -> None:
        response = await async_client.deployments.rollback.with_raw_response.rollback(
            "deploymentId",
        )
    ...
    async def test_streaming_response_rollback(self, async_client: AsyncVaif) -> None:
        async with async_client.deployments.rollback.with_streaming_response.rollback(
            "deploymentId",
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"
    ...
    async def test_path_params_rollback(self, async_client: AsyncVaif) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `deployment_id` but received ''"):
            await async_client.deployments.rollback.with_raw_response.rollback(
                "",
            )
