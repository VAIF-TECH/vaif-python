...
class TestBootstrap:
    ...
    def test_method_list(self, client: Vaif) -> None:
        bootstrap = client.bootstrap.list()
        assert bootstrap is None

    @parametrize
    def test_raw_response_list(self, client: Vaif) -> None:
        response = client.bootstrap.with_raw_response.list()

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        bootstrap = response.parse()
        assert bootstrap is None

    @parametrize
    def test_streaming_response_list(self, client: Vaif) -> None:
        with client.bootstrap.with_streaming_response.list() as response:
...
class TestAsyncBootstrap:
    ...
    async def test_method_list(self, async_client: AsyncVaif) -> None:
        bootstrap = await async_client.bootstrap.list()
        assert bootstrap is None

    @parametrize
    async def test_raw_response_list(self, async_client: AsyncVaif) -> None:
        response = await async_client.bootstrap.with_raw_response.list()

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        bootstrap = await response.parse()
        assert bootstrap is None

    @parametrize
    async def test_streaming_response_list(self, async_client: AsyncVaif) -> None:
        async with async_client.bootstrap.with_streaming_response.list() as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"
