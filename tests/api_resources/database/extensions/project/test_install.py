...
class TestInstall:
    ...
    def test_method_install(self, client: Vaif) -> None:
        install = client.database.extensions.project.install.install(
    ...
    def test_raw_response_install(self, client: Vaif) -> None:
        response = client.database.extensions.project.install.with_raw_response.install(
            project_id="projectId",
            extension_id="x",
        )
    ...
    def test_streaming_response_install(self, client: Vaif) -> None:
        with client.database.extensions.project.install.with_streaming_response.install(
            project_id="projectId",
            extension_id="x",
        ) as response:
    ...
    def test_path_params_install(self, client: Vaif) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `project_id` but received ''"):
            client.database.extensions.project.install.with_raw_response.install(
                project_id="",
                extension_id="x",
            )
...
class TestAsyncInstall:
    ...
    async def test_method_install(self, async_client: AsyncVaif) -> None:
        install = await async_client.database.extensions.project.install.install(
            project_id="projectId",
            extension_id="x",
        )
        assert_matches_type(object, install, path=["response"])
    ...
    async def test_raw_response_install(self, async_client: AsyncVaif) -> None:
        response = await async_client.database.extensions.project.install.with_raw_response.install(
            project_id="projectId",
            extension_id="x",
        )
    ...
    async def test_streaming_response_install(self, async_client: AsyncVaif) -> None:
        async with async_client.database.extensions.project.install.with_streaming_response.install(
            project_id="projectId",
            extension_id="x",
        ) as response:
            assert not response.is_closed
    ...
    async def test_path_params_install(self, async_client: AsyncVaif) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `project_id` but received ''"):
            await async_client.database.extensions.project.install.with_raw_response.install(
                project_id="",
                extension_id="x",
            )
