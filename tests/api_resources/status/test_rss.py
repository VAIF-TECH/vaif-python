...
class TestRss:
    ...
    def test_method_list(self, client: Vaif) -> None:
        rss = client.status.rss.list()
        assert rss is None

    @parametrize
    def test_raw_response_list(self, client: Vaif) -> None:
        response = client.status.rss.with_raw_response.list()
    ...
    def test_streaming_response_list(self, client: Vaif) -> None:
        with client.status.rss.with_streaming_response.list() as response:
...
class TestAsyncRss:
    ...
    async def test_method_list(self, async_client: AsyncVaif) -> None:
        rss = await async_client.status.rss.list()
        assert rss is None

    @parametrize
    async def test_raw_response_list(self, async_client: AsyncVaif) -> None:
        response = await async_client.status.rss.with_raw_response.list()

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        rss = await response.parse()
        assert rss is None

    @parametrize
    async def test_streaming_response_list(self, async_client: AsyncVaif) -> None:
        async with async_client.status.rss.with_streaming_response.list() as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"
