...
class TestCount:
    ...
    def test_method_list(self, client: Vaif) -> None:
        count = client.status.subscribers.count.list()
        assert count is None

    @parametrize
    def test_raw_response_list(self, client: Vaif) -> None:
        response = client.status.subscribers.count.with_raw_response.list()
    ...
    def test_streaming_response_list(self, client: Vaif) -> None:
        with client.status.subscribers.count.with_streaming_response.list() as response:
...
class TestAsyncCount:
    ...
    async def test_method_list(self, async_client: AsyncVaif) -> None:
        count = await async_client.status.subscribers.count.list()
        assert count is None

    @parametrize
    async def test_raw_response_list(self, async_client: AsyncVaif) -> None:
        response = await async_client.status.subscribers.count.with_raw_response.list()
    ...
    async def test_streaming_response_list(self, async_client: AsyncVaif) -> None:
        async with async_client.status.subscribers.count.with_streaming_response.list() as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"
